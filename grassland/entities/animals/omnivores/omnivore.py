...
class Omnivore(Animal):
    ...
    def eat(self, food):
        ...
        if isinstance(food, Carcass):
            food.reduce_hunger(self)       # 사체에서 에너지 회복
            food.being_eaten_by = self     # 사체에 현재 먹는 동물 기록
            self.action_text = "eat_carcass"
        else:
            super().eat(food)

    def decide_food(self, world):
        # diet_preference 확률로 사체/식물 중 하나 선택
    ...
    def flee_or_fight(self, threat, world, dt):
        ...
        if random.random() < self.aggression and self.distance_to(threat) < 42:
            ...
            self.move_away_from(threat.position, self.speed * 1.15)  # 속도 115%로 도주
