...
#
#  [실행 흐름에서의 위치]
#    동물이 die() 하면 world.spawn_carcass() 가 이걸 만든다. 육식·잡식·독수리가 먹이로 삼고,
#    하이에나가 being_eaten_by 를 보고 탈취한다. update() 가 부패·소멸을 처리한다.
...
class Carcass(Resource):
    def __init__(self, position, amount=85.0):
        ...
        self.decomposition_timer = 30.0   # [변수] 안 먹혀도 이 시간 뒤부터 서서히 분해
        self.rot_timer = 8.0              # [변수] 양이 0 이 된 뒤 완전히 사라지기까지(부패)
        self.carried_by = None            # [변수] 누군가 옮기는 중인지(현재 None 고정)
        self.being_eaten_by = None        # [변수] 현재 먹는 포식자(하이에나 탈취 판정의 핵심)

    def consume(self, amount):
        """먹힌 만큼 양만 줄인다(여기선 삭제 안 함 — 소멸은 update 의 부패 처리에 맡김)."""
        # [문법] Resource.consume 을 오버라이드 — 0이 돼도 즉시 delete 하지 않는다(부패 연출 위해).
        taken = min(self.amount, amount)
        self.amount -= taken
        return taken

    def update(self, world, dt):
        """부패 처리: 다 먹힌 사체는 rot_timer 후 소멸, 방치된 사체는 서서히 분해."""
        # [←호출] world.update() 5단계.
        if not self.alive:
            return
    ...
    def reduce_hunger(self, animal):
        """동물이 한 입 먹는다 — 먹은 만큼 그 동물의 허기를 줄인다(덕 타이핑)."""
        # [←호출] Animal.eat / Carnivore.eat 에서 food.reduce_hunger(self).
